...
class NewsModule {
    ...
    private fun getNewsById(tid: Int, nid: Int): News {
        return news.getNews(nid, tid) ?: refreshNewsById(tid, nid)
    }
    ...
    private fun refreshHeadlines(): ArrayList<Headlines> {
        ...
        doc1.select(".index_top_news_newslist").forEach {
            val item = Headlines()
            it.select("a").attr("href").let { content ->
                item.id = parseTid(content)
                    ?: return@forEach
                val matcher = Pattern.compile("[^t]id=(\\d+)")
                    .matcher(content)
                if (!matcher.find()){
                    Log.d("id获取失败")
                    return@forEach
    ...
    private fun parseTid(url: String): Int? {
        val matcher = Pattern.compile("tid=(\\d+)")
            .matcher(url)
        if (!matcher.find()){
