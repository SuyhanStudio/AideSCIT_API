...
import io.github.sgpublic.aidescit.api.exceptions.InvalidRefreshTokenException
...
class TokenUtil private constructor(private val token: TokenPair){
    ...
    companion object {
        ...
        fun startVerify(token: TokenPair): TokenUtil {
            ...
            val bodyPre = TokenBuilder().also {
                it.password = result.password
                it.time = result.tokenCreateTime
            }
            val accessBody = MD5Util.encode(bodyPre.also {
                it.type = TokenBuilder.ACCESS
            })
            ...
            val refreshBody = MD5Util.encode(bodyPre.also {
                it.type = TokenBuilder.REFRESH
            })
        ...
        fun create(username: String, password: String): TokenPair {
            ...
            token.access = "${MD5Util.encode(bodyPre.also {
                it.type = TokenBuilder.ACCESS
            })}.$header."
            token.refresh = "${MD5Util.encode(bodyPre.also {
                it.type = TokenBuilder.REFRESH
            })}."
        ...
        class TokenBuilder: Serializable {
            var password: String = ""
            var time: Long = 0
            val key: String = TokenProperty.TOKEN_KEY
